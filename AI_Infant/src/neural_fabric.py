...
class Neuron:
    ...
    def step(self, current_time: float) -> bool:
        fired = False
        if self.activation_potential >= self.threshold: fired=True; self.last_fired_time=current_time; self.activation_history.append(current_time); self.total_energy_joules+=1e-9; self.activation_potential=0.0
...
class Synapse:
    ...
    def update_weight_hebbian(self, learning_rate: float = 0.01): self.weight=min(1.0, self.weight + learning_rate); self.last_update_joules=5e-10
...
class NeuralFabric:
    ...
    def add_neurons(self, n: int, zone: str):
        if len(self.neurons) + n > self.max_neurons: raise ValueError("Exceeded max_neurons capacity.")
    ...
    def step_simulation(self):
        ...
        if self.rolling_avg_power > self.power_budget_watts:
            raise PowerBudgetExceededError(f"Power budget exceeded: {self.rolling_avg_power:.4f}W > {self.power_budget_watts:.4f}W")
    ...
    def update_power_estimate(self):
        ...
        total_idle_watts = len(self.neurons) * 1e-12; total_synapse_joules = 0
        with self.synapse_lock:
            for source_map in self.synapses.values():
                for synapse in source_map.values():
                    total_synapse_joules += synapse.last_update_joules; synapse.last_update_joules = 0
    ...
    def to_networkx(self):
        G = nx.DiGraph();
        for uid, neuron in self.neurons.items(): G.add_node(uid, zone=neuron.zone, potential=f"{neuron.activation_potential:.2f}")
        with self.synapse_lock:
            for source_uid, targets in self.synapses.items():
                for target_uid, synapse in targets.items():
                    G.add_edge(source_uid, target_uid, weight=round(synapse.weight, 2))
        return G